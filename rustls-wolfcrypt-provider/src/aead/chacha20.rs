...
impl Tls12AeadAlgorithm for Chacha20Poly1305 {
    ...
    fn extract_keys(
        ...
        let iv_array: [u8; NONCE_LEN] = iv.try_into().map_err(|_| UnsupportedOperationError)?;
...
impl core::fmt::Debug for WCTls12Cipher {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("WCTls12Cipher").finish_non_exhaustive()
    }
}
...
impl MessageEncrypter for WCTls12Cipher {
    fn encrypt(
        ...
        // Gather plaintext chunks into a single allocation (one Vec, not a PrefixedPayload
        // staging buffer followed by a .to_vec() copy).
        let plaintext = m.payload.to_vec();
...
impl MessageDecrypter for WCTls12Cipher {
    fn decrypt<'a>(
        ...
        // Allocate only the output buffer; pass ciphertext as a slice reference
        ...
        // The immutable borrow of payload[..message_len] is released before
        // the mutable copy_from_slice below.
...
impl Tls13AeadAlgorithm for Chacha20Poly1305 {
    fn encrypter(&self, key: AeadKey, iv: Iv) -> Box<dyn MessageEncrypter> {
        let key_arr: [u8; 32] = key.as_ref().try_into().expect("ChaCha20 key must be 32 bytes");
...
impl MessageEncrypter for WCTls13Cipher {
    fn encrypt(
        ...
        // Allocate only the output buffer; pass plaintext as a slice reference
        // to avoid an extra heap allocation on the encrypt hot path.
