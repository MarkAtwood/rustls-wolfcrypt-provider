...
const PUB_KEY_SIZE: usize = 1 + COORD_SIZE + COORD_SIZE; // 97
...
impl KeyExchangeSecP384r1 {
    pub fn use_secp384r1() -> Result<Self, rustls::Error> {
        ...
        let curve_size = ECC::get_curve_size_from_id(ECC::SECP384R1)
            .map_err(|_| rustls::Error::General("get_curve_size_from_id failed".into()))?;

        let mut key = ECC::generate_ex(curve_size, &mut rng, ECC::SECP384R1, None, None)
    ...
    pub fn derive_shared_secret(&self, peer_pub_key: &[u8]) -> Result<Box<[u8]>, rustls::Error> {
        ...
        let mut pub_key = ECC::import_unsigned(qx, qy, &[], ECC::SECP384R1, None, None)
