...
const COORD_SIZE: usize = 66;
const PUB_KEY_SIZE: usize = 1 + COORD_SIZE + COORD_SIZE; // 133
...
impl KeyExchangeSecP521r1 {
    pub fn use_secp521r1() -> Result<Self, rustls::Error> {
        ...
        let curve_size = ECC::get_curve_size_from_id(ECC::SECP521R1)
            .map_err(|_| rustls::Error::General("get_curve_size_from_id failed".into()))?;

        let mut key = ECC::generate_ex(curve_size, &mut rng, ECC::SECP521R1, None, None)
