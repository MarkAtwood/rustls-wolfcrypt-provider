...
use wolfssl_wolfcrypt::ecdsa::{
    P256Signature, P256SigningKey, P384Signature, P384SigningKey, P521Signature, P521SigningKey,
};
...
impl TryFrom<&PrivateKeyDer<'_>> for EcdsaSigningKey {
    ...
    fn try_from(value: &PrivateKeyDer<'_>) -> Result<Self, Self::Error> {
        ...
        // pkcs8_parse_err carries any PKCS8 parse diagnostic for use in import_der errors.
        let (sec1_bytes, pkcs8_parse_err): (&[u8], Option<alloc::string::String>) = match value {
            PrivateKeyDer::Pkcs8(der) => {
                ...
                match PrivateKeyInfo::try_from(raw) {
                    ...
                    Err(pkcs8_err) => (raw, Some(pkcs8_err.to_string())),
                }
            }
            PrivateKeyDer::Sec1(der) => (der.secret_sec1_der(), None),
        ...
        let mut ecc = ECC::import_der(sec1_bytes, None, None).map_err(|e| {
            if let Some(ref pkcs8_diag) = pkcs8_parse_err {
                rustls::Error::General(alloc::format!(
                    "ECC::import_der failed: {} (PKCS8 parse also failed: {})",
                    e,
                    pkcs8_diag
                ))
            } else {
                rustls::Error::General(alloc::format!("ECC::import_der failed: {}", e))
            }
        })?;
        ...
        ecc.make_pub(None).map_err(|e| {
            rustls::Error::General(alloc::format!("ECC::make_pub failed: {}", e))
        })?;
        ...
        let scheme =
            x963_len_to_scheme(x963_len).map_err(|e| rustls::Error::General(e.to_string()))?;
        ...
        if priv_buf.is_empty() {
            return Err(rustls::Error::General(
                "export_private returned zero bytes".into(),
            ));
        }

        Ok(Self {
            priv_key: Arc::new(Zeroizing::new(priv_buf)),
            pub_x963: Arc::from(pub_x963),
            scheme,
        })
    }
}
...
impl Signer for EcdsaSigningKey {
    fn sign(&self, message: &[u8]) -> Result<Vec<u8>, rustls::Error> {
        let rng = RNG::new().map_err(|_| rustls::Error::General("RNG::new failed".into()))?;
        ...
        let rs_bytes =
            match self.scheme {
                SignatureScheme::ECDSA_NISTP256_SHA256 => {
                    let pub_arr: [u8; 65] =
                        self.pub_x963.as_ref().try_into().map_err(|_| {
                            rustls::Error::General("pub_x963 length mismatch".into())
                        })?;
                    let d_arr: [u8; 32] =
                        self.priv_key.as_slice().try_into().map_err(|_| {
                            rustls::Error::General("priv_key length mismatch".into())
                        })?;
                    let mut sk =
                        P256SigningKey::import_x963(&pub_arr, &d_arr, rng).map_err(|_| {
                            rustls::Error::General("P256SigningKey::import_x963 failed".into())
                        })?;
                ...
                SignatureScheme::ECDSA_NISTP384_SHA384 => {
                    ...
                    let mut sk =
                        P384SigningKey::import_x963(&pub_arr, &d_arr, rng).map_err(|_| {
                            rustls::Error::General("P384SigningKey::import_x963 failed".into())
                        })?;
                ...
                SignatureScheme::ECDSA_NISTP521_SHA512 => {
                    ...
                    let mut sk =
                        P521SigningKey::import_x963(&pub_arr, &d_arr, rng).map_err(|_| {
                            rustls::Error::General("P521SigningKey::import_x963 failed".into())
                        })?;
                ...
                _ => return Err(rustls::Error::General("Unsupported ECDSA scheme".into())),
            };
...
mod tests {
    ...
    fn test_ecdsa_import_and_sign() {
        ...
        let mut key =
            ECC::generate_ex(curve_size, &mut rng, ECC::SECP256R1, None, None).expect("generate");
...
mod integration_tests {
    use super::*;
    use rustls::pki_types::{PrivatePkcs8KeyDer, PrivateSec1KeyDer};
    ...
    fn make_sec1_p256(priv_bytes: &[u8; 32], pub_bytes: &[u8; 65]) -> alloc::vec::Vec<u8> {
        ...
        let inner_len = version.len() + priv_field.len() + named_curve.len() + pub_key_field.len();

        // Build the DER SEQUENCE header with correct BER/DER length encoding.
        // For lengths ≤ 127 a single byte suffices; 128–255 use the one-byte
        // long form; above 255 use the two-byte long form.  Lengths > 65535
        // are impossible for EC key sizes.
        let mut der = if inner_len <= 0x7f {
            alloc::vec![0x30, inner_len as u8]
        } else if inner_len <= 0xff {
            alloc::vec![0x30, 0x81, inner_len as u8]
        } else {
            alloc::vec![0x30, 0x82, (inner_len >> 8) as u8, inner_len as u8]
        };
