...
impl Signer for RsaSigner {
    fn sign(&self, message: &[u8]) -> Result<Vec<u8>, rustls::Error> {
        ...
        match self.scheme {
            ...
            // pss_sign calls wc_RsaPSS_Sign with the default salt length
            // (WC_RSA_PSS_SALT_LEN_DEFAULT = -1), which means saltLen = hLen.
            // RFC 8446 §4.2.3 mandates saltLen == hLen for TLS 1.3 RSA-PSS;
            // RFC 8017 §9.1 also uses hLen as the default.  If a future wolfssl
            // wrapper exposes an explicit salt_len parameter, it must be set to
            // the hash output length (32 / 48 / 64 bytes for SHA-256/384/512).
            SignatureScheme::RSA_PSS_SHA256
            | SignatureScheme::RSA_PSS_SHA384
            | SignatureScheme::RSA_PSS_SHA512 => {
                let mut rsa = parse_private_key(der)?;
                let mut rng =
                    RNG::new().map_err(|_| rustls::Error::General("RNG init failed".into()))?;
                rsa.set_rng(&mut rng)
            ...
            SignatureScheme::RSA_PKCS1_SHA256 => pkcs1_sign_with_size::<Sha256, 256>(der, message)
                .or_else(|_| pkcs1_sign_with_size::<Sha256, 384>(der, message))
                .or_else(|_| pkcs1_sign_with_size::<Sha256, 512>(der, message)),
            SignatureScheme::RSA_PKCS1_SHA384 => pkcs1_sign_with_size::<Sha384, 256>(der, message)
                .or_else(|_| pkcs1_sign_with_size::<Sha384, 384>(der, message))
                .or_else(|_| pkcs1_sign_with_size::<Sha384, 512>(der, message)),
            SignatureScheme::RSA_PKCS1_SHA512 => pkcs1_sign_with_size::<Sha512, 256>(der, message)
